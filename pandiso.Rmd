...
for (i in 1:NW) {
  ...
  empirical[i] = sum(cov_data[1:k, 2])
  # Assign the sum the cases from rows [j,k] to ith time step
  j = (i-1)*W + 1
  level[i] = mean(cov_data[j:k, 3])
}
...
dfdt$estimated <- estimated # same as cbind(dfdt, estimated)

# subplot in 2 rows
par(mfrow=c(2,1))
# 1st plot
plot(dfdt$time, dfdt$estimated, xlab="", ylab="cases")
points(dfdt$time, dfdt$empirical, pch=20)
legend("topleft", c("empirical", "estimated"), pch = c(1,20))
title("Estimation of cases")
# 2nd plot
plot(seq(from=1,to=length(lvels_dt),by=1), 
     lvels_dt,
     ylim=c(min(lvels_dt),max(lvels_dt)),
     xlab="time", 
     ylab="isolation level")
