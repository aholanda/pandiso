...
# Assign TRUE to update plots in the preprint directory
pandiso.update.preprint.plots <- FALSE
pandiso.plot.write <- function(file_prefix, plt) {
...
The value of `pandiso.time.step` (see \@ref{preface})
...
pandiso.lambda <- pandiso.fit.r * mean(dfdt$level)
...
## Subplot 1
p1 <- ggplot(dfdt) + 
      geom_point(aes(time, empirical), pch = 19) +
      geom_point(aes(time, estimated), pch = 19, col = "gray49") +
      labs(title = paste("Time step=", pandiso.time.step, " days", sep = ""),
           x = "", y = "cases") +
      theme_minimal()
#### add y axis
#  # TODO: plug with y values
...
pandiso.plot.write("estimation", pandiso.estimation.plot)
