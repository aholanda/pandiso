...
``S\~{a}o Paulo Plan'', but from another location 
...
the COVID-19 spread behavior. 
The plot the cumulative number of cases of COVID-19 
...
\def\plateau{6,048,050} It`s striking to notice the consistent increase in the number of cases 
with the plateau to be achieved when the number of cases reaches $\plateau$ 
...
$r=0.043$, $N_m=138,380$, $K=\plateau$. 
The Pearson correlation between the empirical 
and fitted data is $0.996$ and 
 $\text{p-value} < 2.2\times 10^{-16}$.  
Other measures are
the number of time $steps=105$, 
initial $t_{0}=$``2020/03/01''
and final $t_{f}=$``2020/08/06'' 
dates.}
\label{fig:fit}
\end{figure}
